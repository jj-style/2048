...
class BoardCell():
    def __init__(self,x,y):
        self.x = x
        self.y = y
        self.width = ((app.getWidth() - ( (BOARDX+1) * 10 )) / BOARDX)
        self.height = ((app.getHeight() - ( (BOARDY+1) * 10 )) / BOARDY)
        self.value = 1
    def getImage(self):
        pygame.draw.rect(app.getScreen(),CELLCOLOURS[self.value],(self.x,self.y,self.width,self.height),0)
        if self.value != 1:
            pg.renderText(str(self.value),20,BLACK,self.x + self.width/2 - 5,self.y + self.height / 2,app.getScreen())
    def getAnimateImage(self,x,y):
        pygame.draw.rect(app.getScreen(),CELLCOLOURS[self.value],(x,y,self.width,self.height),0)
        if self.value != 1:
            pg.renderText(str(self.value),20,BLACK,x + self.width/2 - 5,y + self.height / 2,app.getScreen())
...
def setBoard():
    ...
    for i in range(BOARDY):
        for k in range(BOARDX):
            ...
            x+= ((app.getWidth() - ( (BOARDX+1) * 10 )) / BOARDX) + 10
        x = 10
        y+= ((app.getHeight() - ( (BOARDY+1) * 10 )) / BOARDY) + 10
...
def gameOver():
    while True:
        ...
        pg.renderText("GAME OVER",50,WHITE,(app.getWidth()//2)/3,(app.getHeight()//2)/4,app.getScreen())
        pygame.display.update()
